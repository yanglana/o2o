$(function () {
    ...
    var infoUrl = "/o2omaven/shopadmin/getproductbyid?productId=" + productId;
    // 获取当前店铺设定的商品类别列表的URL
    var categoryUrl = "/o2omaven/shopadmin/getproductcategorylist";
    // 更新商品信息的URL
    var productPostUrl = "/o2omaven/shopadmin/modifyproduct";
    // 由于商品添加和编辑使用的是同一个页面，
    // 该标识符用来标明本次是添加还是编辑操作
    var isEdit = false;
    if (productId) {
        ...
        getCategory();
        productPostUrl = "/o2omaven/shopadmin/addproduct";
    }
        ...
        $.getJSON(
            infoUrl, function (data) {
                if (data.success) {
                    ...
                    optionArr.map(function (index, domElement) {
                        var isSelect = optionSelected === index.productCategoryId ? 'selected' : '';
                        optionHtml += '<option data-value="' + index.productCategoryId + '" ' + isSelect + '>'
    ...
    $('#submit').click(function () {
        ...
        // 遍历商品详情图控件，获取里面的文件流
        $('.detail-img').map(function (index, domElement) {
            // 判断该控件是否已选择了文件
    ...
    $('#back').on('click', function () {
        window.location.href = "/o2omaven/shopadmin/productmanagement";
    });
});